...
dataloaders = datasets.datasets.get_dataloaders("waterbirds", False)
...
fine_tuned_weights_path = "resnet50_finetuned_final_unfrozen_augmented_200epoker.pth"  # Path to the saved weights
...
input_image, label, bb_coordinate = next(iter(test_loader))
...
original_image = utils.show_image(input_image[0],'./figs/dataset_image.png',plot= False)
...
superimposed_image = np.uint8(0.6 * heatmap_color + 0.4 * original_image)
...
#ax2.add_patch(rect) 
plt.savefig("heat_map_guidedteach_oodtest_bb_box1")
# Show the plot
#plt.show()
